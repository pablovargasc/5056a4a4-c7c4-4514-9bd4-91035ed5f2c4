...
-- SELECT DISTINCT te regresa los valores unicos
SELECT
    gender,
    count(gender)
FROM
...
WHERE
    ...
    -- LOWER(gender) != 'undefined' / NOT LIKE / LIKE '%male'
GROUP BY gender

-- EX.3) Get the percentage of male/female tasters.
-- :: cambia de tipo de dato, (entera, decimal)
...
-- where aplica antes del group by
-- having aplica despues del group by
