...
--SELECT name, DIV(population, area_km2) AS "density" FROM country WHERE area_km2 >0 ORDER BY density DESC LIMIT 10;

-- EX.2) Get the count of male/female tasters.
--SELECT gender, count(gender) FROM taster GROUP BY gender;
--SELECT gender, count(*) FROM taster WHERE LOWER(gender) != 'undefined' GROUP BY gender;
--SELECT gender, count(*) FROM taster WHERE LOWER(gender) LIKE '%male' GROUP BY gender;
--SELECT gender, count(*) FROM taster WHERE LOWER(gender) NOT LIKE 'undef%' GROUP BY gender;

-- EX.3) Get the percentage of male/female tasters.
--WITH taster_valid AS (SELECT * FROM taster WHERE LOWER(gender) IN ('male', 'female')), taster_gender_agg AS (SELECT gender, COUNT(*)::NUMERIC(7,2) gender_sum FROM taster_valid GROUP BY gender), taster_total AS (SELECT count(*)::NUMERIC(7, 2) total FROM taster_valid) SELECT gender, TRUNC(100*gender_sum/total,2) percentage FROM taster_gender_agg, taster_total;
...
--SELECT avg(happiness_score) FROM country WHERE LOWER(split_part(gdp_usd, ' ', 2)) IN ('billion', 'trillion');
...
--SELECT name, population, area_km2, gdp_usd, happiness_score FROM country WHERE name IN ('Canada', 'France', 'Germany', 'Italy', 'Japan', 'United Kingdom', 'United States') ORDER BY happiness_score DESC;
...
--SELECT variety, count(variety) AS count_variety FROM wine GROUP BY variety ORDER BY count_variety DESC;

-- E) How many wines are registered per country? Show country name and number of wines, ordered by the latter (desc).
--SELECT name, count(name) AS count_name FROM wine JOIN country c on wine.country_id = c.id GROUP BY name ORDER BY count_name DESC;
